...
name: ruby
description: >-
  Ruby development environment. Use when the project needs this capability or the user / team manifest asks for it. Use for specialized `ruby` work when listed in TEAM.yaml or explicitly requested.
...
Ruby development environment. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `ruby` or user asks by name
...
2. **Confirm tooling** - Verify `ruby` runtime/CLI/MCP is available; document install if missing.
